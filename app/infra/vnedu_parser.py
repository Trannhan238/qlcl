...
def _validate_subject_columns(
    ...
    """
    Validate column counts per subject type.

    Scored: at least 1 column (level_col required, score_col optional)
    Qualitative: at least 1 column (level only)
    """
    for entry in subject_entries:
        ...
        if len(columns_used) == 0:
            raise ValueError(
                f"Subject '{entry.get('subject')}' has no columns "
